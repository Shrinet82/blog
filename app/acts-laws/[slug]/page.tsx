...
import { getActLawBySlugQuery } from "@/sanity/lib/queries";
    ...
    image: ({ value }: any) => {
      ...
      return (
        <div className="my-8 rounded-sm overflow-hidden border border-outline-variant/30 max-h-[600px] flex justify-center bg-surface-variant/20">
          <img
            alt={value.alt || "Act or Law Image"}
            loading="lazy"
...
export default async function ActLawPage({ params }: { params: { slug: string } }) {
  const actLaw: ActLaw | null = await client.fetch(getActLawBySlugQuery, { slug: params.slug });

  if (!actLaw) {
    notFound();
  }

  const formattedDate = actLaw.publishDate 
    ? new Date(actLaw.publishDate).toLocaleDateString("en-US", {
        month: "long",
        day: "numeric",
        year: "numeric"
      })
  ...
  return (
    <article className="w-full max-w-3xl mx-auto px-margin-mobile md:px-0 py-12 md:py-20 min-h-screen">
      <Link href="/acts-laws" className="inline-flex items-center text-sm font-bold text-on-surface-variant hover:text-primary transition-colors mb-12">
        <span className="material-symbols-outlined mr-2 text-[18px]">arrow_back</span>
        Back to all Acts & Laws
      </Link>
      ...
      <header className="mb-12 border-b border-outline-variant/30 pb-12">
        ...
        <div className="bg-surface-variant/20 p-6 md:p-8 rounded-sm border-l-4 border-primary/50 mb-8">
          <h3 className="text-sm font-bold text-on-surface uppercase tracking-wider mb-3">Synopsis</h3>
        ...
        {actLaw.actFileUrl && (
          <div className="flex flex-wrap gap-4 pt-4">
            ...
            >
              ...
              Read Full Document Inline
            </a>
            <a
              href={`${actLaw.actFileUrl}?dl=`}
      ...
      {actLaw.body && actLaw.body.length > 0 && (
        <div className="prose prose-lg max-w-none prose-headings:font-display prose-a:text-primary hover:prose-a:text-primary/80">
          <PortableText value={actLaw.body} components={PortableTextComponents} />
        </div>
      )}

      {actLaw.actFileUrl && (
        <div id="document-viewer" className="mt-16 pt-8 border-t border-outline-variant/30">
          <h2 className="text-3xl font-display font-bold text-on-surface mb-6">Full Document</h2>
          <div className="w-full h-[800px] border border-outline-variant/30 rounded-sm overflow-hidden bg-surface-variant/10 shadow-sm">
            <iframe 
              src={`${actLaw.actFileUrl}#view=FitH`} 
              className="w-full h-full" 
              title="Act/Law PDF Document"
            />
          </div>
        </div>
      )}
